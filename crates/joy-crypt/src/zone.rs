...
//!
//! Per-member access is granted by wrapping the zone key under a KEK
//! derived from a pairwise X25519 ECDH between the granter's identity
//! and the recipient's identity (ADR-038, JOY-0157-86). Going through
//! identity material that is stable across passphrase rotation (ADR-039)
//! means passphrase changes do not invalidate Crypt access.
//!
...
//!
//! These are the primitive zone/blob operations; the joy item, auth-token
//! and Project models live in joy-core, which re-exports this module's API
//! from `joy_core::crypt` and keeps the domain glue (active-session context
//! and `platform_zone_keys`) there.
...
/// derived from the plaintext so re-sealing an unchanged event yields
/// byte-identical output: the log is content-addressed and a re-save
/// produces no new git objects. Safe because a content-derived nonce
/// only repeats for identical plaintext.
pub fn encrypt_blob_with_nonce(
    ...
    let ct = crate::aead::seal(zone_key.as_bytes(), nonce, &aad, plaintext)
...
///
/// Returns [`Error::ZoneKeyUnavailable`] when the lookup closure has no
/// key for the blob's zone; joy-core maps that back to its domain
/// `ZoneAccessDenied` at the crate boundary.
pub fn decrypt_blob(
    zone_key_lookup: impl Fn(&str) -> Option<ZoneKey>,
    blob: &[u8],
) -> Result<(String, Vec<u8>), Error> {
    if blob.len() < 8 + 1 + 1 + 12 + 16 || &blob[..8] != FILTER_MAGIC {
        return Err(Error::Auth("not a Crypt blob".into()));
    }
    let version = blob[8];
    ...
    if blob.len() < zone_end + 12 + 16 {
        return Err(Error::Auth("truncated Crypt blob".into()));
    }
    let zone_name = std::str::from_utf8(&blob[zone_start..zone_end])
        .map_err(|_| Error::Auth("invalid zone name in Crypt blob".into()))?
        .to_string();
    ...
    let zone_key = zone_key_lookup(&zone_name).ok_or_else(|| Error::ZoneKeyUnavailable {
        zone: zone_name.clone(),
    })?;
    let aad = aad_for(zone_name.as_bytes());
    let plaintext = crate::aead::open(zone_key.as_bytes(), &nonce, &aad, ct)
        .map_err(|_| Error::Auth(format!("failed to decrypt zone '{}'", zone_name)))?;
...
pub fn unwrap_for_member(
...
) -> Result<ZoneKey, Error> {
    let bytes =
        hex::decode(wrap_hex).map_err(|e| Error::Auth(format!("invalid crypt wrap: {e}")))?;
    if bytes.len() < 32 {
    ...
    let plain = wrap::unwrap(&kek, &bytes[32..])
        .map_err(|_| Error::Auth(format!("failed to unwrap zone {zone_name}")))?;
    let arr: [u8; 32] = plain
        .try_into()
        .map_err(|v: Vec<u8>| Error::Auth(format!("zone key has wrong length: {}", v.len())))?;
...
mod tests {
    ...
    #[test]
    fn blob_missing_zone_key_is_zone_key_unavailable() {
        let zk = ZoneKey::generate();
        let blob = encrypt_blob("default", &zk, b"x");
        let err = decrypt_blob(|_| None, &blob).unwrap_err();
        assert!(matches!(err, Error::ZoneKeyUnavailable { zone } if zone == "default"));
    }
    ...
    fn blob_rejects_tampered_zone_name() {
        ...
        // Flip a byte inside the zone name (offset 10). The lookup returns
        // a key for any name, so the tampered AAD makes AEAD open fail.
        blob[10] ^= 1;
